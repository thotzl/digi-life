...
export function runSimulation(cfg: FullConfig, totalTicks = 10000): {
  ...
  for (let tick = 0; tick < totalTicks; tick++) {
    ...
    creatures.forEach(agent => {
      ...
      const alphaRot = (torque - rotDragCoeff * agent.omegaRot) / momentOfInertia;

      agent.omegaRot += alphaRot * frameScale;
      agent.headingAngle = Math.atan2(Math.sin(agent.headingAngle + agent.omegaRot * frameScale), Math.cos(agent.headingAngle + agent.omegaRot * frameScale));
      ...
      agent.vx = (agent.vx + (fx - dragForward * vForward * Math.cos(agent.headingAngle) + dragLateral * vLateral * Math.sin(agent.headingAngle)) / mass * frameScale) * 0.94;
      agent.vy = (agent.vy + (fy - dragForward * vForward * Math.sin(agent.headingAngle) - dragLateral * vLateral * Math.cos(agent.headingAngle)) / mass * frameScale) * 0.94;
      agent.omegaRot *= 0.88;
